...
class Obenland_Wp_Approve_User extends Obenland_Wp_Plugins_v301 {
	...
	public function wpau_approve( $user_id ) {
		...
		if ( $this->options['wpau-send-approve-email'] AND ! get_user_meta( $user_id, 'wp-approve-user-mail-sent', true ) ) {
			...
			$sent = @wp_mail(
				$user->user_email,
				sprintf( _x( '[Donate for a date 2013] Tài khoản đã được xác nhận', 'Blogname', 'wp-approve-user' ), $blogname ),
				$this->populate_message( $this->options['wpau-approve-email'], $user ), $headers
			);
	...
	public function delete_user( $user_id ) {
		...
		if ( $this->options['wpau-send-unapprove-email'] ) {
			...
			@wp_mail(
				$user->user_email,
				sprintf( _x( '[Donate for a date 2013] Tài khoản đã bị xóa', 'Blogname', 'wp-approve-user' ), $blogname ),
				$this->populate_message( $this->options['wpau-unapprove-email'], $user ), $headers
			);
