...
function retrieve_password() {
	global $wpdb, $current_site, $phpmailer;
    //BinhLV
  ...
  //  $newPass = wp_generate_password(8, false);
   // wp_set_password($newPass, $user_ID);
    $message = 'Chào bạn <strong>'.$user_login."</strong>,<br/><br/>";
	$message .= __('Một người nào đó yêu cầu được thiết lập lại mật khẩu cho tài khoản tại website: '). network_home_url() . "<br/><br/>";	
	$message .= sprintf(__('Username: %s'), $user_login) . "<br/><br/>";    
    $message .= __('Nếu đây không phải là yêu cầu của bạn thì bạn bỏ qua email này và không có vấn đề gì xẩy ra'). "<br/>";
    $message .= __("Để thiết lập lại mật khẩu của bạn, hãy truy cập vào địa chỉ dưới đây:<br/>");
    $message .= home_url("/reset-mat-khau/?key=$key&login=" . rawurlencode($user_login)). "<br/><br/>";    
    $message .= __('Trân trọng!');
